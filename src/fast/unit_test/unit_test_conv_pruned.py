import sys
sys.path.append('/Users/maxiu/github/CNNumpy/')
from src.fast.layers import Conv, AvgPool
from src.fast.utils import mask_filter
import numpy as np
import torch
...
def test_conv():
    ...
    nf, cf, hf, wf = 4, 10, 3, 3 # Filters.
    filter_index = [0,3]
    kept_index = [x for x in range(0, nf) if x not in filter_index]

    x = np.random.randn(n, c, h, w)
    x = x.astype('float64')
    W = np.random.randn(nf, cf, hf, wf)
    W = W.astype('float64')
    W = mask_filter(W,  kept_index=kept_index)
    b = np.random.randn(nf)
    b = b.astype('float64')
    b = mask_filter(b,  kept_index=kept_index)
    deltaL = np.random.rand(n, len(kept_index), h-hf+1, h-hf+1) # Assuming stride=1/padding=0.

    # CNNumpy.
    inputs_cnn = x
    weights_cnn, biases_cnn = W, b
    conv_cnn = Conv(nb_filters=nf, filter_size=hf, nb_channels=cf, stride=1, padding=0, pruned_index=filter_index)
    ...
    conv_pt = nn.Conv2d(c, len(kept_index), hf, stride=1, padding=0, bias=True)
